        ...
        .on("click selectfile  unselectall mouseenter mouseleave", function(){
            ...
            if(files.length){
              let paths = files.map(item => fm.path(item.hash))
							 paths = paths.map(path => "/public/files/" +  path.replace(/\\/g,"/"))
							let allTmpFiles = files.map(item => {
								return `${$("#user-media-url").val()}/.tmb/${item.tmb}`
							})
              $("#path-text-location").val(paths)
              $("#path-location").val(paths)
              $("#path-location").attr("data-small-image", `${$("#user-media-url").val()}/.tmb/${files[0].tmb}`)
              $("#path-location").attr("data-small-image-all-tmb", allTmpFiles)
            } else{
